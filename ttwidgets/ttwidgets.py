...
class TTWidget(tk.Frame):
    ...
    def _enter(self, event=None, *a, **kw):
        ...
        if bx_state:
            ...
            if platform.system() != 'Windows' and self._widget_cget(state_s) == tk.NORMAL:
                self._activate()
        return
    ...
    def _widget_cget(self, option, **kwargs):
        ...
        if cook_b:
            if False:
                pass
            # <border object: >
            elif option in (activebackground_s, background_s, font_s, highlightbackground_s, ):
                value = str(value)
            # <color object: >
            elif option in (activeforeground_s, disabledforeground_s, foreground_s, highlightcolor_s, ):
                value = str(value)
            # <font object: >
            elif option in (font_s, ):
                value = str(value)  
            # <index object: >
            elif option in (anchor_s, compound_s, default_s, justify_s, relief_s, state_s, ):
                value = str(value)  
            # <pixel object: >
            elif option in (borderwidth_s, highlightthickness_s, padx_s, pady_s, wraplength_s, ):
                value = str(value)  
            # int
            elif option in (height_s, repeatinterval_s, repeatdelay_s, underline_s, width_s, ):
                value = int(value)  
            # str
            elif option in (bitmap_s, command_s, cursor_s, image_s, overrelief_s, takefocus_s, text_s, textvariable_s, ):
                if option == command_s:
                    if self.command:
                        value = self.command
