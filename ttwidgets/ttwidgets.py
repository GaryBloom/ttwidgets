...
b_seqs = (B1_Motion_s, Bx_Motion_s,)
SHIFT_KEY = 0x0001  # 1
CAPS_LOCK = 0x0002
CTRL_KEY = 0x0004  # 4
# ALT_L     = 0x0008
...
ttfont_dict_keys = (family_s, size_s, weight_s, slant_s, funderline_s, foverstrike_s)

font_dict_keys = (family_s, size_s, weight_s, slant_s, underline_s, overstrike_s)

case_dict_keys = (case_s, upper_s, lower_s, title_s, swapcase_s, capitalize_s)

event_types = ('Activate', 'B1-', 'B2-', 'B3-', 'B4-', 'B5-', 'Button-', 'ButtonPress-', 'ButtonRelease-', 'Colormap', 'Configure', 'Deactivate', 'Destroy', 'Enter', 'Expose', 'FocusIn', 'FocusOut', 'Key', 'KeyPress', 'KeyRelease', 'Leave', 'Map', 'Motion', 'MouseWheel', 'Property', 'Return', 'Unmap', 'Visibility')
...
def _flesh_config(widget, cfg, **kw):
    ...
    pared_b = kw.pop('pared', False)
    d = cfg.copy()
    od = {}
    if isinstance(widget, TTButton) or isinstance(widget, TTLabel):
...
def _print_out(widget, text_w, *args, **kwargs):
    ...
    screen_b = kwargs.pop('screen', getattr(widget, '_print_cfg_screen_b', False)) # True)) #
    ...
    if text_w and text_b:
        ...
        text_w.see(tk.END)
    if screen_b or not text_w:
        args and widget and widget.after_idle(lambda *a: print(*args, **kwargs))
    if raise_b:
        print(*args, file=output, **kwargs)
        stringy = output.getvalue()
...
def alias(option=None):
    ...
    In general, the alias of an option is:
       * for simple words, the first three letters (e.g. 'anc' for 'anchor', 'cur'
         for 'cursor') unless there is a common abbreviation for the word (like
         'cmd' for 'command', 'cpd' for 'compound', 'wt' for 'weight', etc.), and
       * for compound-word options, the first letter of each word (i.e. 'abg' for
         'activebackground', 'rd' for 'repeatdelay', etc.)
...
def convert_font_dict_to_ttoptions_dict(d):
    ...
    'underline' option.
    '''
    return {('f'+k if k in (underline_s, overstrike_s) else k):v for k,v in d.items()}
...
def convert_ttoptions_dict_to_font_dict(d):
    ...
    '''
    return {(k[1:] if k in (funderline_s, foverstrike_s) else k):v for k,v in d.items()}
...
def gen_tag_attrs(widget=None, options_d=None, font=None, case=None, **kwargs):
    ...
    Keyword Arguments:
        ...
        extend:     include 'text' as an attribute in the output. Default is False.
    ...
    if widget:  # and isinstance(widget, TTWidget): #
        ...
        w_options_d = {k:v[-1] for k,v in widget.config().items() if len(v) == 5 and str(v[-1]) != str(v[-2]) and k not in excludes_t}
        try:
    ...
    d = _merge_dicts(w_options_d, convert_font_dict_to_ttoptions_dict(w_font_d), options_d, convert_font_dict_to_ttoptions_dict(font_d), kwargs)
    bad_opts = []
    for key, val in d.items():
        ...
        if False:
            pass
        elif key3 in (bg_s, background_s[:3], fg_s, foreground_s[:3]) or key2 == underline_s[:2] or kalias in (bg_s, fg_s, underline_as):
            fmt_s += '%s=%s ' % (keyout, val)
        elif key2 in (bitmap_s[:2], image_s[:2], ) or kalias in (bitmap_as, image_as):
            fmt_s += '%s=%s ' % (keyout, val)
            ...
            if '%s=%s ' % (auto_bd, 1) in fmt_s:
                if val != 1:
                    fmt_s = fmt_s.replace('%s=%s ' % (auto_bd, 1), '%s=%s ' % (keyout, val))
            else:
                fmt_s += '%s=%s ' % (keyout, val)
        elif key4 in (compound_s[:4], ) or kalias == compound_as:
            if '%s=%s ' % (auto_cpd, tk.CENTER) in fmt_s:
                if val != tk.CENTER:
                    fmt_s = fmt_s.replace('%s=%s ' % (auto_cpd, tk.CENTER), '%s=%s ' % (keyout, val))
            else:
                fmt_s += '%s=%s ' % (keyout, val)
        ...
        elif key[:7] in (sbg_s, selectbackground_s[:7], sbd_s, selectborderwidth_s[:7], sfg_s, selectforeground_s[:7], ):
            fmt_s += '%s=%s ' % (keyout, val)
        # special for fonts
            ...
            fmt_s += '%s=%d ' % (kfunc(tk_font.BOLD), 1 if isinstance(val, str) and val.lower() == tk_font.BOLD else 0)
        elif key2 == slant_s[:2]:
            fmt_s += '%s=%d ' % (kfunc(tk_font.ITALIC), 1 if isinstance(val, str) and val.lower() == tk_font.ITALIC else 0)
        elif key3 in (funderline_as, funderline_s[:3]):
            fmt_s += '%s=%d ' % (kfunc(funderline_s), 1 if str(val) in ('1', 'True') else 0)
        elif key3 in (foverstrike_as, foverstrike_s[:3]):
            fmt_s += '%s=%d ' % (kfunc(foverstrike_s), 1 if str(val) in ('1', 'True') else 0)
        # special "case" implementation
        elif key3 in (case_s[:3]):
            ...
            fmt_s += '%s=%s ' % (keyout, val)
    if bad_opts:
        _print_out(widget, text_w, 'EXCEPTION: UNEXPECTED TAG ATTRS: %r' % bad_opts, Raise=True)
    fmt = fmt_s.strip()
    if widget and isinstance(widget, TTWidget) and recurse_b:
        fmt = [fmt,]
        for name, gathering in widget._get_kids(items=True):
            child = gathering['label']
            case = gathering.get(case_s, '')
            kid_options = {k:v[-1] for k,v in child.config().items() if len(v) == 5 and str(v[-1]) != str(v[-2]) and (k, v[-1]) not in w_options_d.items() and not (k in label_override_d and str(v[-1]) == "0")} #
            ...
            if cf != cdf:
                ...
                c_font_d = {}
            case and kid_options.update(case=case)
            fmt.append(gen_tag_attrs(options=kid_options, font=c_font_d, **kwargs))
    return fmt if index_i is None else fmt[index_i]

def get_font_dict(f):
    '''Return a dict of font attributes for font F.

    Standard font attributes include: family, size, weight, slant, underline, overstrike.
    '''
...
def parse_tag_attrs(tag_str, options_d=None, font_d=None, case='',
                    **kwargs):
    '''
    Splits tagged-text tag attributes from TAG_STR into standard Tkinter and custom
    TTWidgets widget options.
    ...
    for keyval in split_attrs(tag_str):
        ...
        if val == 'None': # in ('False', 'None') #
            pass
        elif key3 in (bg_s, background_s[:3], fg_s, foreground_s[:3], ) or kalias in (bg_s, fg_s):
        ...
        elif key4 in (command_s[:4], compound_s[:4], ) or kalias in (command_as, compound_as):
            options_d.update(**{key: val})
        elif key2 in (height_s[:2], width_s[:2]) or key3 in (repeatdelay_s[:3], repeatinterval_s[:3]) or kalias in (height_as, width_as, repeatdelay_as, repeatinterval_as):
            options_d.update(**{key: int(val)})
        elif key2 in (cursor_s[:2], ) or key3 == font_s[:3] or kalias in (cursor_as, font_as):
            options_d.update(**{key: val})
        ...
        # special for TTListbox
        elif key[:7] in (sbg_s, selectbackground_s[:7], sfg_s, selectforeground_s[:7], ) or kalias in (selectbackground_as, selectforeground_as):
            options_d.update(**{key: val})
        # special for fonts
            ...
            try:
                ...
                _print_out(widget, text_w, 'EXCEPTION: ERROR Setting Font Size to %r' % val, Raise=True)
        elif key3 in (bold_as, tk_font.BOLD[:3]) or kalias == bold_as:
            font_d[weight_s] = tk_font.BOLD if str(val) not in ('0', 'False', ) else tk_font.NORMAL
        elif key2 in (weight_s[:2], ) or kalias == weight_as:
            font_d[weight_s] = val
        elif key2 in (italic_as, tk_font.ITALIC[:2]) or kalias == italic_as:
            font_d[slant_s] = tk_font.ITALIC if str(val) not in ('0', 'False', ) else tk_font.ROMAN
        elif key2 in (slant_s[:2], ) or kalias == slant_as:
            font_d[slant_s] = val
        elif key3 in (funderline_as, funderline_s[:3]) or kalias == funderline_as:
            font_d[underline_s] = 1 if str(val) not in ('0', 'False', ) else 0
        elif key3 in (foverstrike_as, foverstrike_s[:3]) or kalias == foverstrike_as:
        ...
        # special "case" implementation
            ...
            for s in (upper_s, capitalize_s, lower_s, title_s, swapcase_s):
                if s.startswith(lowval):
                    ...
                    break
        elif key2 == upper_s[:2] or key3 in (capitalize_s[:3], ) or kalias in (upper_as, capitalize_as):
    ...
    if bad_opts:
        _print_out(widget, text_w, 'EXCEPTION: UNEXPECTED TAG ATTRS: %r' % bad_opts, Raise=True)
    if attr_b:
        return case if attr_b == case_s else options_d.get(attr_b, font_d.get(attr_b))
...
def split_attrs(s):
    ...
    for f in re.split(r'(\s+)', s)[::-1]:
        if not quoted_b and f.endswith(('"', "'")):
            ...
            not f.isspace() and outfields.append(f)
        else:  # quoted
            partial.append(f)
            if quoted_b in f:
...
def split_chunk(chunk):
    ...
    - the 'tag' and 'attributes' strings may be empty if the CHUNK consists of
      only plain text,
    - the 'attributes' string may be empty if there were no attributes passed, and
    ...
    if chunk.lower().startswith('<t') and chunk.endswith('/>'):
        ...
        chunk = '<{tag} {new_attrs}>{text}</{tag}>'.format(tag=tag, new_attrs=new_attrs, text=text)
    matches = sentinel_d['repatt2'].findall(chunk)
    result = Chunk(*matches[0]) if len(matches) == 1 else Chunk("", "", chunk) if chunk else ()
    return result
...
def split_dict_into_options_fontattrs_and_case(d):
    ...
    dict.
    '''
    c_d = {case_s:(v if k == case_s else k) for k,v in d.items() if k in case_dict_keys}
    f_d = {(k[1:] if k in (funderline_s, foverstrike_s) else k):v for k,v in d.items() if k in ttfont_dict_keys}
    o_d = {k:v for k,v in d.items() if k not in ttfont_dict_keys + case_dict_keys}
...
def split_tagged_text_into_chunks(text):
    ...
    if not sentinel_d.get('repatt1'):
        patt1 = r'(<t(?:ag)?.*?(?<=/)(?:t(?:ag)?)?>)'
        sentinel_d.update(repatt1=re.compile(patt1, flags=re.IGNORECASE | re.DOTALL))
    return [chunk for chunk in sentinel_d['repatt1'].split(text) if chunk]

def strip_tags(text, *a, **kw):
    '''Strip away all tags from TEXT and return the printable text.

    Returns a string without any tagging information.
    '''
    return ''.join([split_chunk(chunk).text for chunk in split_tagged_text_into_chunks(text)])
...
def wrap_tagged_text(text, count=0):
    ...
    wrapped_text = text
    if count > 0:
        divs = [split_chunk(chunk) for chunk in split_tagged_text_into_chunks(text)]
        ...
        wrapped_s = '\n'.join([textwrap.fill(s, count) for s in total_lines]) + ('\n' if final_nl_b else '')
        # now have to map the wrapped_string to the divs!
        insert_pts = []
        divx = x = tl = wi = 0
        div = divs[divx]
        dl = len(div.text)
        for tsi, tsc in enumerate(total_string):
            if wrapped_s[wi] != tsc:
        ...
        if insert_pts:
            ...
            for ipt in insert_pts[::-1]:
                _, divx, x, _, sp = ipt
                divs_l[divx][TEXT] = '{0}\n{1}'.format(divs[divx][TEXT][:x], divs[divx][TEXT][x + 1 if sp else x:])
            divs_t = [Chunk(*div) for div in divs_l]
            chunks = [('<t {0}>{1}</t>'.format(div.attrs, div.text) if div.attrs else div.text) for div in divs_t if div and div.text]
...
class TTWidget(tk.Frame):
    ...
    visual schemes.
        ...
        Sections support the following Label widget options [shown with (Alias)]:
                background      (BG)
                bitmap          (BIT)
                borderwidth     (BD)
                compound        (CPD)
    ...
    The TTWidget is meant to act exactly like a regular Tkinter Button/Label, other than the special formatting accepted in the 'text' attribute.
    ...
    Known Issues:
        - Bug: image centered under a multi-text text is obscured by the text's
          ...
          corresponding text.
    Ignored:
        - Widget width is not set "properly" (according to charCnt * width of "0")
    ...
    if 'widget class attributes':
        OPTIONS_NOT_IMPLEMENTED = ()
        #
        #
        opts_in_button_not_in_label = (command_s, default_s, overrelief_s, repeatdelay_s, repeatinterval_s)
        methods_in_button_not_in_label = 'flash invoke'
        opts_in_label_not_in_button = ()
        opts_in_frame_not_in_widget = (class_s, colormap_s, container_s, visual_s)
        #
        opts_in_checkandradiobutton_not_in_button = ('indicatoron', 'offrelief', 'selectcolor', 'selectimage', 'variable')
        methods_in_checkandradiobutton_not_in_button = 'deselect select'
        opts_in_checkbutton_not_in_button = opts_in_checkandradiobutton_not_in_button + ('offvalue', 'onvalue', )
        methods_in_checkbutton_not_in_button = 'toggle'
        opts_in_radiobutton_not_in_button = opts_in_checkandradiobutton_not_in_button + ('value', ) #
        opts_in_listbox_not_in_button = ()
        methods_in_listbox_not_in_button = 'activate bbox curselection delete get index insert itemcget itemconfig nearest scan_dragto scan_mark see select_anchor select_clear select_includes select_set selection_anchor selection_clear selection_includes'
        #
        widget_opts_to_base_cfg = (bitmap_s, borderwidth_s, compound_s, cursor_s, height_s, highlightbackground_s, highlightcolor_s, highlightthickness_s, image_s, relief_s, takefocus_s, width_s, )
        widget_opts_to_base_pack = (padx_s, pady_s)
        widget_opts_to_kids_cfg = (activebackground_s, activeforeground_s, disabledforeground_s, foreground_s, font_s, justify_s, state_s, wraplength_s, )
        widget_opts_to_kids_pack = () # should be empty
        widget_opts_to_base_and_kids = (anchor_s, background_s, )
        widget_opts_req_procreation = (bitmap_s, compound_s, image_s, text_s)
        widget_opts_for_custom_impl = widget_opts_req_procreation + (state_s, textvariable_s, underline_s, ) # not passed to Frame
        widget_opts_to_base = widget_opts_to_base_cfg + widget_opts_to_base_pack
        widget_opts_to_kids = widget_opts_to_kids_cfg + widget_opts_to_kids_pack
        #
        ...
        #
        #
        frame_pack_def_options_d = {anchor_s:tk.CENTER, expand_s:False, fill_s:tk.NONE, in_s:None, ipadx_s:0, ipady_s:0, padx_s:0, pady_s:0, side_s:tk.TOP}

        priority_lists = [(image_s, bitmap_s, text_s), (textvariable_s, text_s)]

        event_types_requiring_recursion = (e for e in event_types if e.startswith('B'))
        event_types_precluding_recursion = (e for e in event_types if e not in event_types_requiring_recursion)

        _instances = []

        _internal_bound_event_patterns_ = ('<Enter>', '<B1-Enter>', '<B%d-Enter>'%RMB, '<Leave>', '<B1-Leave>', '<B%d-Leave>'%RMB, '<Button-1>', '<ButtonRelease-1>', '<Button-%d>'%RMB, '<ButtonRelease-%d>'%RMB, '<B1-Motion>', '<B%d-Motion>'%RMB, )
        _internal_bound_event_types_ = ('Button', 'ButtonRelease', 'Enter', 'Leave', 'Motion')
    ...
    def __init__(self, master=None, widget=None, widget_class=None, **options):
        ...
        if TTWidget.button_def_options is None:
            TTWidget.button_def_options = self._get_widget_default_options(tk.Button)
        if TTWidget.label_def_options is None:
            TTWidget.label_def_options = self._get_widget_default_options(tk.Label)
        if TTWidget.frame_def_options is None:
            TTWidget.frame_def_options = self._get_widget_default_options(tk.Frame)
        if TTWidget.widget_option_aliases_d is None:
            ...
            for key in self.widget.keys():
                if key not in TTWidget.widget_option_aliases_d and key not in TTWidget.widget_option_aliases_d.values():
                    TTWidget.widget_option_aliases_d[key[:3]] = key
            TTWidget.widget_option_unaliases_d = {v:k for k,v in TTWidget.widget_option_aliases_d.items()}
            TTWidget.widget_option_unaliases_d.update(**{background_s:bg_s, foreground_s:fg_s, borderwidth_s:bd_s})
        #
        frame_options = {k:v[-1] for k,v in self._widget_config().items() if len(v) == 5 and k in self.frame_def_options}
        ...
        self.font_d = {}
        # self.font = get_named_font(self._widget_cget(font_s), create=True, modify=False)
        self.observer = None
        self.textvariable = None
        self.debug_text = options.pop('debug_text', None)
        #
        self.options, self.font_d = options, {}
        for opt, val in list(self._reorder_dict(self.options).items()) + list(self.font_d.items()): #
    ...
    def __repr__(self):
        options = {k:v[-1] for k,v in self._widget_config().items() if len(v) == 5 and str(v[-1]) != str(v[-2])}
        frame_options = {k:v[-1] for k,v in super().config().items() if len(v) == 5 and str(v[-1]) != str(v[-2]) and k in self._widget_config().keys()}
        if frame_options:
    ...
    def _activate(self, *args, **kwargs):
        ...
        store_b and self._widget_config(state=tk.ACTIVE)
        return

    def _base_config(self, **options):
        debug_b = options.get('debug', False)
        debug_b and self._print("\n_BASE_CONFIG(self={self},  **{options})".format(self=self, options=options))
        caller = options.pop('caller', '')  # UNUSED
        debug_b = options.pop('debug', False)
        frames_b = options.pop('frames', True) # False) #
        base_opts = {k:v for k,v in options.items() if k in (bg_s, background_s, cursor_s)}
        if frames_b:
            for frame in [getattr(self, '_compoundframe', None), getattr(self, '_textframe', None)] + getattr(self, '_subframes', []):
                frame and frame.winfo_exists() and frame.config(**base_opts)
    ...
    def _child_config(self, child, **options):
        debug_b = options.get('debug', False)
        debug_b and self._print("\n_CHILD_CONFIG(self={self}, child={child}\n  **{options})".format(child=child, self=self, options=options))
        caller = options.pop('caller', '')  # UNUSED
        debug_b = options.pop('debug', False)
        ...
        if vals_d:
            ...
            if attrs:
                options, font_d, _ = self.parse_tag_attrs(attrs, options, font_d)
            if font_d:
    ...
    @staticmethod
    def _config_pared(widget, *args, **kwargs):
        return {k:v[-2:] for k,v in widget.config().items() if len(v) == 5 and str(v[-2]) != str(v[-1])}
    ...
    def _disable(self, *args, **kwargs):
        ...
        store_b and self._widget_config(state=tk.DISABLED)
        return

    def _discipline_family(self, *a, **kw):
        ...
        if self.widget_class == tk.Button:
            self._widget_bind('<Button-1>', self._press, recurse=True, **kw) # ) #'+',
            self._widget_bind('<ButtonRelease-1>', self._release, recurse=True, **kw) # ) # '+',
        self._widget_rebind_externals(**kw)

    def _enable(self, *args, **kwargs):
        store_b = kwargs.get('store', True)
        for kid in self._get_kids():
            kid.config(state=tk.NORMAL)
        bg = self._widget_cget(background_s)
        self._base_config(bg=bg)
        store_b and self._widget_config(state=tk.NORMAL)
        return

    def _enter(self, event=None, *a, **kw):
        bx_state = kw.get('bx_state', getattr(event, state_s, 0) & (MOUSE_B1 | MOUSE_B2 | MOUSE_B3))
        debug_b = kw.get('debug', self.default_debug)
        if event and self != event.widget:
            debug_b and self._print("_ENTERing: IGNORED! DISPARITY/CHILD! Self={s}, Event={e}, event.widget={ew}, bx_state={bx}".format(s=self, e=event, ew=event.widget if event else 'N/A', bx=bx_state))
            return
        debug_b and self._print("_ENTERing: Self={s}, Event={e}, event.widget={ew}, bx_state={bx}".format(s=self, e=event, ew=event.widget if event else 'N/A', bx=bx_state))
        if bx_state:
            ...
            overrelief = self._widget_cget(overrelief_s) if self.widget_class == tk.Button else None
            if overrelief:
                super().config(relief=overrelief)
            if platform.system() == 'Darwin' and self._widget_cget(state_s) == tk.NORMAL:
                self._activate()
        return
    ...
    def _get_frame_def_opts(self, **kwargs):
        return {k:v for k,v in self.frame_def_options.items() if k not in self.opts_in_frame_not_in_widget}

    def _get_opts_for_base(self, **kwargs):
        return {k:v for k,v in self.options.items() if k in (self.widget_opts_to_base_cfg + self.widget_opts_to_base_and_kids) and k in self.frame_def_options} # opts_in_frame_not_in_widget}

    def _get_opts_for_kids(self, **kwargs):
        return {k:v for k,v in self.options.items() if k in (self.widget_opts_to_kids_cfg + self.widget_opts_to_base_and_kids) and not k.startswith(text_s)}
    ...
    def _indicate_default(self, on_b=None, bd=None, color=None, **kwargs):
        ...
        if on_b:
            super().config({highlightbackground_s: color, highlightcolor_s: color, highlightthickness_s: bd})
        else:
            super().config({highlightthickness_s: 0})

    def _is_mine(self, other, *args, **kwargs):
        result = other in self._get_kids()
        if not result and other:
            result = other in [getattr(self, '_compoundframe', None), getattr(self, '_textframe', None)] + getattr(self, '_subframes', [])
        return result

    def _leave(self, event=None, *a, **kw):
        bx_state = kw.get('bx_state', getattr(event, state_s, 0) & (MOUSE_B1 | MOUSE_B2 | MOUSE_B3))
        debug_b = kw.get('debug', self.default_debug) # False) # True) #
        if event and self != event.widget:
            debug_b and self._print("_LEAVing: IGNORED! DISPARITY/CHILD! Self={s}, Event={e}, event.widget={ew}, bx_state={bx}".format(s=self, e=event, ew=event.widget if event else 'N/A', bx=bx_state))
            return
        debug_b and self._print("_LEAVing: Self={s}, Event={e}, event.widget={ew}, bx_state={bx}".format(s=self, e=event, ew=event.widget if event else 'N/A', bx=bx_state))
        overrelief = self._widget_cget(overrelief_s) if self.widget_class == tk.Button else None
        if bx_state:
    ...
    def _motion(self, event=None, *a, **kw):
        ...
        if not hasattr(self, '_prev_widget'):
            setattr(self, '_prev_widget', None)
        ...
        debug_b and self._print('{fn} Self={s}, Event.Widget={ew} Event.Num={n} EVENT={e} *{a} **{kw}, CURRENT={c}'.format(fn=_func_name().upper(), s=self, ew=event.widget, n=event.num, e=repr(event), a=a, kw=kw, c='SAME' if current_widget == event.widget else current_widget if current_widget else 'NONE'))
    ...
    def _pack_anchored_frame(self, frame, **kwargs):
        ...
        def __pad_frame(self, frame, **kwargs):
            ...
            debug_b and print(text_b, graphic_b)
            if padx is None or pady is None:
                bd = int(str(self._widget_cget(borderwidth_s)))
                padx = int(str(self._widget_cget(padx_s)))
                pady = int(str(self._widget_cget(pady_s)))
                if text_b:
                    font = tk_font.Font(font=self._widget_cget(font_s))
                    w, h = font.measure('0'), font.metrics("linespace")
                    padx = 10 * w // 20 # 20 #
                    pady = 10 * h // 60 # 50 #
                    debug_b and print('FONT METRICS:', font, w, h, padx, pady)
                else:
                    pass
                ipadx = factor(bd)
                ipady = factor(bd)
            ...
            debug_b and print('PADDING IS ', repr(padx), repr(pady))
    ...
    def _press(self, event=None, *a, **kw):
        debug_b = kw.get('debug', self.default_debug)
        num = kw.get('num', getattr(event, 'num', -1))
        event_widget = self._resolve_widget(event.widget)
        debug_b and self._print('_PRESS: B{n} Self={s}, Event.Widget={ew}, Event={e}'.format(n=num, s=self, ew=event.widget, e=event))
        ...
        if self == event.widget:
            for mod in ('B1', 'B%d'%RMB):
                self._widget_bind('<%s-Enter>'%mod, self._enter, recurse=False)
                self._widget_bind('<%s-Leave>'%mod, self._leave, recurse=False)
            ...
            self._bx_bindings[B1_Motion_s] = event.widget.bind(B1_Motion_s, self._motion)
            self._bx_bindings[Bx_Motion_s] = event.widget.bind(Bx_Motion_s, self._motion)
        else:
            pass
        self._fired_b = False
    ...
    def _print(self, *args, **kwargs):
        return _print_out(self, getattr(self, 'debug_text', None), *args, **kwargs)
    ...
    def _procreate(self, master=None, **options):
        ...
        Note that a single widget is used internally to store state information.
        If only a single child widget would be created, the creation of the child
        widget is skipped and the internal state widget is mapped instead.
        '''
        bind_b = options.pop('bind', False)
        debug_b = options.pop('debug', self.default_debug)
        debug_b and self._print("PROCREATING for {0}!".format(self))
        ...
        text_b = text
        text_chunks = [tc for tc in split_tagged_text_into_chunks(text_b) if tc]
        ...
        if not self.emulation_b:
            ...
            if text_b and is_tagged_text(text_b):
                ...
                options, font_d, case = self.parse_tag_attrs(chunk_tags, self.options.copy(), base_font_d.copy())
                if font_d:
            ...
            self.widget.lift()
            debug_mode_b and self.widget.config(bg='magenta')
        ...
        if not options:
            options = _merge_dicts(self._get_opts_for_kids(), label_override_d, {compound_s:compound})
        frame_options = {k:v for k,v in options.items() if k in self.frame_def_options}
        ...
        if text_b and is_tagged_text(text_b) and len(text_chunks) > 1:
            text = text_b
            wraplength = self._widget_cget(wraplength_s)
            if wraplength > 0:
            ...
            #
            debug_b and self._print('TEXT is {0!r}'.format(text))
            chunks = split_tagged_text_into_chunks(text)
            row = column = 0
            debug_b and self._print('CHUNKS are %r' % chunks)
            self._textframe = tk.Frame(self._compoundframe)
            self._subframes = [tk.Frame(self._textframe)]
            for chunk in chunks:
                debug_b and self._print('CHUNK is %r' % chunk)
                if not chunk:
                    continue
                _tag, chunk_tags, chunk_text = split_chunk(chunk)
                label_options, font_d, case = self.parse_tag_attrs(chunk_tags, options.copy(), base_font_d.copy())
                if font_d:
                    temp_font = tk_font.Font(**font_d)
                ...
                debug_b and self._print('CHUNK_LINES is %r' % chunk_lines)
                ...
                for i, line in enumerate(chunk_lines):
                    ...
                    if end_nl_f:
                        line = line[:-1]
                    Label = tk.Label # TTLabel #
                    lab = Label(self, text=line, **label_options)
                    gathering[str(lab)] = dict(label=lab, index=len(gathering), type=text_s, row=row, column=column, text1=orig_line, text2=line, attrs=chunk_tags, font_d=font_d, font=temp_font, options=label_options, case=case)
                    if not suppress_f:
                        lab.pack(in_=self._subframes[-1], side=tk.LEFT, fill=tk.BOTH, expand=True)
                    debug_b and self._print('PUTTING %r at %d,%d' % (line, row, column))
            ...
            debug_mode_b and super().config(highlightthickness=2, highlightcolor='magenta', highlightbackground='magenta', bg='cyan')
            debug_mode_b and self._textframe.config(highlightthickness=2, highlightcolor='teal', highlightbackground='teal')
            self._textframe.grid(in_=self._compoundframe, row=trow, column=tcol)
            self._textframe.config(**frame_options)
            for f in self._subframes:
                f.grid(**layout_options)
                f.config(**frame_options)
                debug_mode_b and f.config(highlightthickness=2, highlightcolor='magenta', highlightbackground='magenta')
                layout_options['row'] += 1
            underline = self._widget_cget(underline_s)
            if underline >= 0:
                self._underline(underline, gathering)
        if graphic_b or not self.emulation_b and (not text_b or not is_tagged_text(text_b) or len(text_chunks) == 1):
            key = None
            gathered = {}
            font_d = {}
            temp_font = None
            ...
            if not self.emulation_b:
                if not text_b:
                    ...
                    _tag, chunk_tags, chunk_text = split_chunk(text_b)
                    options, font_d, case = self.parse_tag_attrs(chunk_tags, options, base_font_d.copy())
                    if font_d:
                        temp_font = tk_font.Font(**font_d)
                        options[font_s] = temp_font
                    case_func = self._get_case_func(case)
                    ...
                    chunk_tags, chunk_text = "", text_b
                options[text_s] = chunk_text
                gathered.update(text=options[text_s], options=options, text1=text_b, text2=chunk_text, attrs=chunk_tags, font_d=font_d, font=temp_font, )
                key = text_s if key is None else compound_s
            gl = tk.Label(self, **options)
            text_rows = len(self._subframes) if hasattr(self, '_subframes') else 1
            layout_options = dict(in_=self._compoundframe, row=grow, column=gcol)
            rowspan = text_rows if compound in (tk.LEFT, tk.RIGHT, tk.CENTER) else 1
            if rowspan:
        ...
        anchor = self._widget_cget(anchor_s)
        self._pack_anchored_frame(self._compoundframe, anchor=anchor, text=text_b, graphic=graphic_b)
        self._indicate_default()
        debug_b and self._print('GATHERING is %r' % gathering)
        self._discipline_family(kids=gathering)
        return gathering

    def _release(self, event=None, *a, **kw):
        debug_b = kw.get('debug', self.default_debug)
        num = kw.get('num', getattr(event, 'num', -1))
        if self._widget_cget(state_s) == tk.DISABLED:
            return
        ...
        resolved_current = self._resolve_widget(current_widget)
        event_widget = self._resolve_widget(event.widget)  # UNUSED
        debug_b and self._print('_RELEASE: B{n} Self={s}, Event.Widget={ew}, Event={e}, CurrentWidget={cw}'.format(n=num, s=self, ew=event.widget, e=event, cw=current_widget))
        ...
        if self._valid_widget(self._depressed_w):
            if self._is_mine(self._depressed_w):
                ...
                if bind_widget and bind_widget == self._depressed_w:
                    for seq, funcid in self._bx_bindings.items():
                        debug_b and self._print("TRYING TO UNBIND Seq {seq!r} with FuncID {funcid!r} from Widget {widget!r}".format(seq=seq, funcid=funcid, widget=self._depressed_w))
                        self._depressed_w.unbind(seq, funcid)
        self._depressed_w = None
        return self._enable()

    @staticmethod
    def _reorder_dict(d, *a, **kw):
        od = collections.OrderedDict({k:v for k,v in sorted(d.items(), reverse=False) if 'debug' in k})
        od.update({k:v for k,v in sorted(d.items(), reverse=True) if k.startswith(text_s)})
        od.update({k:v for k,v in sorted(d.items()) if k not in od})
        return od

    def _repeat_click(self, *a, **kw):
        ...
        if self._depressed_w:
            ...
            if repeatinterval:
                self.after_id = self.after(int(repeatinterval), self._repeat_click)
        else:
            self.after_id = None

    @classmethod
    def _resolve_widget(cls, widget, **kw):
    ...
    def _trace_callback(self, varname=None, varindex=None, varmode=None, *a, **kw):
        if varmode == 'w':
            value = self.getvar(varname)  # UNUSED
            self._kids = self._procreate()

    def _underline(self, pos=-1, gathering=None, *args, **kwargs):
        store_b = kwargs.get('store', True)
        store_b and self._widget_config(underline_s, pos)
        if not self.emulation_b:
            return
        ...
        if pos >= 0:
            count = 0
            for kid, vals in gathering.items():
                prev_count = count
    ...
    def _widget_bind(self, sequence=None, func=None, add=None, *a, **kw):
        ...
        child widgets and track it so we can clean up if the user tries to unbind.

        This routine makes all bindings 'adds' but keeps track to simluate user
        non-adds.
        '''
        debug_b = kw.get('debug', self.default_debug)
        debug_b and self._print("_WIDGET_BIND({self}, {sequence}, {func}, {add}, *{a}, **{kw}) Called!".format(self=self, sequence=sequence, func=func, add=add, a=a, kw=kw))
        if sequence == func is None:
            return super().bind() if self.emulation_b else self.widget.bind()
        if func is None:
            return super().bind(sequence) if self.emulation_b else self.widget.bind(sequence)
        ...
        if kidsonly:
            ...
            if self.emulation_b:
                func_id = super().bind(sequence, func, '+', *a)
            else:
                func_id = self.widget.bind(sequence, func, '+', *a)
            self._funcids_d[sequence][func_id] = dict(sequence=sequence, func=func, add=add, internal=internal_b, self=func_id, kids={}, frames={})
            func_ids = [func_id, ]
        for frame in [getattr(self, '_compoundframe', None), getattr(self, '_textframe', None)] + getattr(self, '_subframes', []):
            if frame and frame.winfo_exists():
                fid = frame.bind(sequence, func, '+', *a)
                frame_str = str(frame)
                self._funcids_d[sequence][func_id]['frames'][frame_str] = fid
                func_ids.append(fid)
        matching_recursion_events = (e for e in self.event_types_requiring_recursion if e.startswith(sequence[1:]))
        recursion_b = recurse_b or matching_recursion_events or not internal_b and False
        if recursion_b or kidsonly:
            for child_str, vals in kids_d.items():
                child = vals['label']
                debug_b and self._print("BINDING {sequence} to Child={child}: Child.bind({sequence}, {func}, {add}, *{a}, **{kw})".format(child=child, sequence=sequence, func=func, add=add, a=a, kw=kw))
                try: # if True: #
                    if kidsonly:
                        cfid = self._funcids_d[sequence][func_id]['kids'].get(child_str)
                        cfid and child.unbind(sequence, cfid)
                    child_func_id = child.bind(sequence, func, '+', *a)
                    self._funcids_d[sequence][func_id]['kids'][child_str] = child_func_id
                    func_ids.append(child_func_id)
                except: # else: #
                    self._print("EXCEPTION while Binding Child({c}, {s}, {f}, {ad}, *{a})".format(c=child, s=sequence, f=func, ad=add, a=a))
        result = func_ids if internal_b else func_ids[0]
        debug_b and self._print('FUNC_IDS are ', func_ids, ' internal_b is ', internal_b)
        return result

    def _widget_cget(self, option, **kwargs):
        '''Get config option info from the internal widget used to keep state.'''
        ...
        if cook_b:
            ...
            elif option in (borderwidth_s, highlightthickness_s, padx_s, pady_s, wraplength_s, ):
                value = int(str(value))
            elif option in (activebackground_s, activeforeground_s, anchor_s, background_s, compound_s, default_s, disabledforeground_s, font_s, foreground_s, highlightbackground_s, highlightcolor_s, justify_s, relief_s, state_s, ):
                value = str(value)
            else:
                pass
        return value
    ...
    def _widget_config(self, option=None, value=sentinel, **kwargs):
        ...
        pared_b = kwargs.pop('pared', False)  # UNUSED
        success_i = 0
        ...
        for option, value in kwargs.items():
            try: # if True: #
                self.widget.config(**{option:value})
                success_i += 1
            except: # else: #  TclError:
                self._print("EXCEPTION While Configuring {o} with {v}".format(o=option, v=value), Raise=True)
        if value == sentinel:
            ...
            if not option:
                ...
                self._print("EXCEPTION: Option {o} Not Found in Widget".format(o=option), Raise=True)
            return results
        return success_i == count_i

    def _widget_rebind_externals(self, *a, **kw):
        '''Rebind any bindings that existed before the child widgets were created.'''
        if not hasattr(self, '_funcids_d'):
            setattr(self, '_funcids_d', {})
        values_l = self._funcids_d.values() # bypasses seqs
        events_l = (list(v.values())[0] for v in values_l)
        external_events_l = (e for e in events_l if e.get('internal') is False)  # == 0)
        for e in external_events_l:
            self._widget_bind(e['sequence'], e['func'], e['add'], internal=e['internal'], kidsonly=e['self'], **kw)

    def _widget_unbind(self, sequence, funcid=None, *a, **kw):
        ...
        recurse_b = kw.get('recurse', False)
        debug_b and self._print("_WIDGET_UNBIND({self}, {s}, {f}, {a}, {kw})".format(self=self, s=sequence, f=funcid, a=a, kw=kw))
        if not hasattr(self, '_funcids_d'):
            setattr(self, '_funcids_d', {})
        func_id = None  # UNUSED
        x_funcids = ()
        ...
        if seq_funcids_d:
            funcids = [fid for fid in seq_funcids_d.keys() if seq_funcids_d[fid].get('internal') == internal_b]
            if funcid is True:
                if internal_b:
                    if len(funcids) > 1:
                        raise Exception("Failure Trying to Find Internal FuncID for Sequence {sequence}. Got {funcids}".format(sequence=sequence, funcids=funcids))
            ...
            for fid in x_funcids:
                try:
                    if self.emulation_b:
                        fid and super().unbind(sequence, fid)
                    else:
                        fid and self.widget.unbind(sequence, fid)
                except:  # TypeError:
                    self._print('EXCEPTION for unbind(sequence={sequence}, funcid={fid})'.format(sequence=sequence, fid=fid), Raise=True)
                d = seq_funcids_d.pop(fid)
                # self.widget.unbind(sequence, func, *a)
                kids = d.get('kids', {})
                if recurse_b or kids:
                    for child_str in kids:
                        cfid = kids[child_str]
                        child = self._valid_widget(child_str)
                        debug_b and self._print("SELF is ", self, ", CHILD_STR is ", child_str, ", CHILD is ", child, ", CFID is ", cfid)
                        debug_b and self._print("_UNBINDING Child: child={child}, seq={sequence}, funcid={cfid})".format(child=child_str, sequence=sequence, cfid=cfid))
                        if child and cfid:
                            child.winfo_exists() and child.unbind(sequence, cfid)
        return x_funcids
    ...
    def bind_all(self, sequence=None, func=None, add=None, *a):
    ...
    def cget2(self, key, *a, **kw):
        ...
        '''
        kw.update(cook=True)
        return self._widget_cget(key, *a, **kw)

    def config(self, cnf=None, *args, **kwargs):  # cnf=None,
        ...
        if cnf and type(cnf) in (dict, collections.OrderedDict):
            kwargs.update(cnf)
        ...
        if args:
            ...
            if arg0 in (class_s, TTWidget, 'TTWidget', TTLabel, 'TTLabel', TTButton, 'TTButton'):
                ...
                for opt, val in kwargs.items():
                    setattr(cls, opt, val)
                    debug_b and self._print("CONFIG Class %s SETTING %s to %r" % (cls, opt, val), getattr(cls, opt))
                ...
                results = []
                _subframes = (self._subframes[i] for i in arg1 if i < len(self._subframes)) if arg1 else self._subframes
                for f in _subframes:
                ...
                _kids = (_get_kids[i] for i in arg1 if i < len(_get_kids)) if arg1 else _get_kids
                for child in _kids:
                ...
                self._print("EXCEPTION: Config Arg {0} Not Supported".format(arg0), Raise=True)
            if len(args) > 2 or len(args) > 1 and arg0lower not in ('subframes', 'subbase', 'child', 'kid'):
                self._print("EXCEPTION: Config Args {0} Ignored".format(args[1:]), Raise=True)
        abstain_b = kwargs.pop('abstain', False)
        debug_b = kwargs.pop('debug', self.default_debug)
        init_b = kwargs.pop('init', False)  # UNUSED
        ...
        if not args and kwargs:
            for (key, val) in kwargs.items():
                ...
                if key in ttfont_dict_keys:
                    fkey = key[1:] if key in (funderline_s, foverstrike_s) else key
                ...
                if key in self.OPTIONS_NOT_IMPLEMENTED:
                    self._print('EXCEPTION: Widget Option {0} Not Implemented'.format(key), Raise=True)
                elif key in (case_s, ):
                    case_func = self._get_case_func(val)
                    if not self.emulation_b:
                        text = self.widget.text
                        self._widget_config(text=case_func(text))
                    for name, gathering in self._get_kids(items=True):
                        child = gathering['label']
                        ...
                        self._child_config(child, caller='config', **{text_s:text})
                    ...
                    if self.widget_class is not tk.Button:
                        continue
                    debug_b and self._print('SELF ', self, ', KEY=', key, ', VAL=', val)
                    if False:
                        pass
                    elif key == command_s:
                        self.command = val
                    elif key == default_s:
                        'capture keybd focus and bind <Return> and <Space> to invoke()?'
                        self._indicate_default()
                        if val in (tk.NORMAL, tk.DISABLED):
                            ...
                            pass
                    elif key in (overrelief_s, repeatdelay_s, repeatinterval_s):
                        pass
                    ...
                    for child in self._get_kids():
                        self._child_config(child, caller='config', **{key:val})
                    ...
                    self._print("EXCEPTION: Unexpected Option {0}!".format(key), Raise=True)
            ...
            # return self._config_pared(self) if pared_b else self._widget_config()
    ...
    def dump(self, stringy='', **kwargs):
        ...
        self._print('+' * 40)
        stringy and self._print(stringy)
        self._print('DUMPING:: SELF {s}, class {cls}, widget_class {wc}, index {i}'.format(s=str(self), cls=self.__class__, wc=self.widget_class, i=self._index_))
        ...
        for num, (name, vals) in enumerate(kids):
            kid = vals['label']
            self._print('DUMPING:: KID[{n}]: {id} type={t!r}'.format(n=num, id=str(kid), t=vals['type']))
    ...
    def flash(self, **kwargs):
        ...
        if alt_b:
            ...
            debug_b and self._print('INIT STATE is %r' % init_state)
            for i in range(3):
                self.after(2*i*wait, lambda:self.config(relief=tk.SUNKEN))
                self.after((2*i+1)*wait, lambda:self.config(relief=tk.RAISED))
            ...
            if init_state != tk.DISABLED:
                flash_funcs = (lambda:self._activate(store=False),
                               lambda:self._enable(store=False))
                for i in range(times):
                    self.after((2*i)*wait, flash_funcs[0 if init_state == tk.NORMAL else 1])
                    self.after((2*i+1)*wait, flash_funcs[1 if init_state == tk.NORMAL else 0])
    ...
    def fromwidget(cls, widget, unmap_b=False, **kw):
        ...
        TTWidget uses an internal widget to store the state of the compound TTWidget.
        This routine allows an existing widget to be passed in and used as the basis
        of a TTWidget.'''
        unmap_b = kw.pop('unmap', unmap_b)
        obj = cls(None, widget, **kw)
        unmap_b and unmap(widget)
        return obj
    ...
    def invoke(self, **kwargs):
        ...
        '''
        debug_b = kwargs.get('debug', self.default_debug)
        debug_b and self._print("INVOKE(self={self})".format(self=self))
        if hasattr(self.widget, 'invoke'):
            debug_b and self._print("INVOKING...")
    ...
    def unbind_all(sequence):
        '''Unbind for all widgets for event SEQUENCE all functions.'''
        return super().unbind_all(sequence)

    def unbind_class(className, sequence):
...
class TTListbox(tk.Listbox):
    ...
    def __init__(self, *a, **kw):
        ...
        self.options, self.font_d, self.case = split_dict_into_options_fontattrs_and_case(kw)
    ...
    def cget(self, key):
        ...
        if key in (text_s, text_as):
            return self.text
        elif key in ttfont_dict_keys:
            return self.font_d.get(key)
        elif key in (case_s, case_as) or key in case_dict_keys:
            return self.case
        ...
        if kw:
            for k, v in kw.items():
                if k in (text_s, text_as):
                    self.text = v
                    store_b and self._insert_new_elements()
    ...
    def gen_tag_attrs(self, *a, **kw):
        ...
        if widget not in (sentinel, None):
            raise Exception("TTListbox.gen_tag_attrs(): unsupported 'widget' keyword value {w}".format(w=widget))
        kw['widget'] = None
    ...
    def insert(self, index, *elements, **kw):
        ...
        if PyVers_f >= 3.4:
            ...
            # there is a bug in earlier Py versions which causes the above routine to silently fail on the first element.
            elems_to_process = elements[::1 if index == tk.END else -1]
            for elem in elems_to_process: # elements[::-1]:
                if type(elem) in (list, tuple):
                    elem1 = [e.replace('{', r'\{').replace('}', r'\}') for e in elem]
        ...
        new, custom TTWidgets options are added, namely:
            ...
            For CASE Processing:
                ...
                case=[one of the below]
                    ...
                    swapcase
        '''
        return sorted(super().keys() + list(ttfont_dict_keys) + [text_s, case_s])
...
class TTToolTip:
    ...
    def __init__(self, widget=None, text='...', **kw):
        self.defaults_d = {background_s:'wheat', borderwidth_s:1, justify_s:tk.LEFT, relief_s:tk.SOLID}
        self.custom_defs = {'base': '', 'delay': 500, ipadx_s: 2, ipady_s: 1, 'offsetx': 2, 'offsety': 2}
        self.base = kw.pop('base', self.custom_defs['base']) # ''/'widget' or 'mouse'/'cursor'
        ...
        if self.widget:
            ...
            self.widget.bind("<ButtonPress-{RMB}>".format(RMB=RMB), self._leave)
        self._aid = None
    ...
    def _cancel(self):
        if self._aid:
            self.widget and self.widget.after_cancel(self._aid)
            self._aid = None
    ...
    def _showtip(self, event=None):
        ...
        if self.base and self.base[:1] in ('mouse'[:1], cursor_s[:1]):
            ...
            x0 = self.widget.winfo_rootx() + self.widget.winfo_width() - self.offsetx
            y0 = self.widget.winfo_rooty() + self.widget.winfo_height() - self.offsety
        if self._top:
            self._top.deiconify()
            ...
            if Platform_s == 'Darwin':
                try:
                    self._top.tk.call("::tk::unsupported::MacWindowStyle", "style", self._top._w, "help", "noActivates")
                except tk.TclError:
                    pass
                except:
                    pass
    ...
    def config(self, cnf=None, **kw):
        ...
        for k, v in kw.items():
            if k in self.ttlabel.keys():
                ...
                raise Exception('Unexpected Key/Val Pair {k}:{v}'.format(k=k, v=v))
        if not cnf and not kw:
            return _flesh_config(self, self.ttlabel.config(), defaults=self.defaults_d, base=('', self.base), delay=(500, self.delay), ipadx=(2, self.ipadx), ipady=(1, self.ipady), offsetx=(2, self.offsetx), offsety=(2, self.offsety))
    ...
    def gen_tag_attrs(self, *a, **kw):
        ...
        if kw.get('widget', sentinel) is not None:
            raise Exception("TTToolTip.gen_tag_attrs(): 'widget' keyword must be set to None")
        return gen_tag_attrs(None, *a, **kw)
    ...
    def parse_tag_attrs(self, tags_str, options_d=None, font_d=None, case='', **kwargs):
        '''See help on method parse_tag_attrs()'''
        return parse_tag_attrs(tags_str, options_d, font_d, case, widget=self, text=getattr(self, 'debug_text', None), **kwargs)
